...
/// conversion step.
/// </summary>
internal sealed class PoeNinjaPriceClient : IDisposable
{
    ...
    internal async Task<PoeNinjaPriceSnapshot> GetPricesAsync(string league, CancellationToken cancellationToken = default)
    {
        ArgumentException.ThrowIfNullOrWhiteSpace(league);
        var cacheKey = league.Trim();
        lock (_sync)
        {
            if (_snapshots.TryGetValue(cacheKey, out var cached) && DateTimeOffset.UtcNow - cached.CreatedAt < TimeSpan.FromMinutes(5))
            {
    ...
    internal static string NormalizeItemName(string itemName) =>
...
internal sealed class PoeNinjaPriceException(string message) : Exception(message);
